...
export class RateLimitGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (user && user._id) {
      ...
      const entry = rateLimitStore.unauthenticated.get(key) || { timestamps: [], blockedUntil: 0 };
      ...
      recent.push(now);
      rateLimitStore.unauthenticated.set(key, {timestamps: recent,blockedUntil: entry.blockedUntil || 0,});
    }
    return true;
  }
}
