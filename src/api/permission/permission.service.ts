...
export class PermissionService {
   constructor(
      @InjectModel(RolePermission.name) private readonly rolePermission: Model<RolePermission>,
    ) {}

   async createPermission(data: PermissionRequestDto, currentUserId: object) {
      const existingPermission = await this.rolePermission.findOne({ permissionKey: data.permissionKey, userId: data.userId });
  ...
  async updatePermission(query: FilterQuery<RolePermission>, data: UpdateQuery<RolePermission>,currentUserId: object ) {
    ...
    return this.rolePermission.findOneAndUpdate(query, updatePayload, { new: true });
  }

  async deletePermission(query: FilterQuery<RolePermission>,currentUserId: object ) {
  ...
  async hasPermission(userId: Object, permissionKey: string): Promise<boolean> {
    const existingPermission = await this.rolePermission.findOne({ userId, permissionKey, isPublished: true, isDeleted :false});
